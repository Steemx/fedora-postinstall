...
echo "=== INICIANDO INSTALACIÓN DEL SISTEMA BASE Y NIRI (FEDORA 44 - DNF5) ==="
echo "=== REPORTE DE SISTEMA BASE ===" > "$LOG_FILE"
echo "Fecha: $(date)" >> "$LOG_FILE"
echo "Usuario: $REAL_USER" >> "$LOG_FILE"
echo "--------------------------------------------" >> "$LOG_FILE"

echo "=== 1. Optimizando DNF y DNF5 ==="
...
echo "=== 5. Instalando herramientas de compresión y utilidades esenciales ==="
...
log_status $? "Base Niri/Noctalia (Git) y configuración de SDDM"

echo "=== 7. Instalando fuentes del sistema y temas ==="
/usr/bin/dnf install -y \
...
echo "=== 8. Configurando Códecs y Multimedia Avanzada (Swap de Free a NonFree) ==="
...
echo "=== 9. Ajustes de Rendimiento Avanzados (ZRAM y sysctl) ==="
...
echo "=== 11. Configurando Distribución de Teclado y Entorno ==="
...
echo "=== 12. Optimización de servicios y arranque ==="
...
echo "=== Comprobaciones de Hardware Finales ===" >> "$LOG_FILE"
echo "Generando initramfs con Dracut..."
/usr/sbin/dracut --force --verbose || echo "⚠️ Advertencia en Dracut"
