...
# Método oficial y nativo de DNF5 en Fedora para habilitar RPM Fusion limpiamente
/usr/bin/dnf install -y fedora-workstation-repositories
/usr/bin/dnf config-manager setopt rpmfusion-free.enabled=1 2>/dev/null || true
/usr/bin/dnf config-manager setopt rpmfusion-nonfree.enabled=1 2>/dev/null || true
...
# Instalación pura sin intermediarios gráficos
