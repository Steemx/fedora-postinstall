#!/usr/bin/env bash
# ==============================================================================
# SCRIPT DE POST-INSTALACIÓN PARA FEDORA LINUX (NIRI + NOCTALIA)
# ==============================================================================
...
# Eliminar paquetes "recortados" de Fedora
...
# Instalar FFmpeg completo desde RPM Fusion
/usr/bin/dnf install -y ffmpeg ffmpeg-libs libavdevice --allowerasing

# Códecs de audio y drivers de video Intel
...
log_status $? "Eliminación de firewalld"

# Configurar teclado latinoamericano global
...
# KDE Connect
sudo -u $REAL_USER bash -c 'cat > $HOME/.config/autostart/org.kde.kdeconnect.daemon.desktop << EOF
[Desktop Entry]
Type=Application
...
# Telegram
sudo -u $REAL_USER bash -c 'cat > $HOME/.config/autostart/org.telegram.desktop.desktop << EOF
[Desktop Entry]
Type=Application
Name=Telegram
...
sudo -u $REAL_USER bash -c 'cat > $HOME/.config/environment.d/qt.conf << EOF
...
sudo -u $REAL_USER bash -c "cat > $HOME/.show_install_log.sh << EOF
#!/usr/bin/env bash
echo '================================================================='
cat '$LOG_FILE'
echo '================================================================='
...
sudo -u $REAL_USER bash -c "cat > $HOME/.config/autostart/show_log.desktop << EOF
[Desktop Entry]
Type=Application
...
echo " Al volver deberías ver el greeter de GDM."
echo " Selecciona la sesión 'Niri' en el engranaje de la esquina inferior."
echo "=============================================================================="
sleep 10
reboot
